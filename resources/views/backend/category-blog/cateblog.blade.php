...
@section('titleheader', 'Category Blog')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <a href="{{route('category-blog.create')}}" class="btn btn-primary">
                    <i class="fas fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;Tambah
                </a>
                <div class="card-tools">

                    <form action="{{route('search.categroyblog')}}">
                ...
                <table class="table table-head-fixed text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$category->name}}</td>
                            <td id="action-31">
                                <a href="{{route('category-blog.edit', $category->id)}}" class="btn btn-info btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i>&nbsp;&nbsp;Ubah</a>
                                <form id="deleteForm-{{$category->id}}" action="{{route('category-blog.destroy', $category->id)}}" style="display:inline-block" method="POST" class="deleteForm-{{$category->id}}">
                                    @method('DELETE')
                                    @csrf
