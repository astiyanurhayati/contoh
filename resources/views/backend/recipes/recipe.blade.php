...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-tools">
                    <form action="{{route('search.recipe')}}">
                ...
                <table class="table table-head-fixed text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><img src="{{asset('recipeimg/'.$data->gambar)}}" alt="" width="100px"></td>
                            <td>{{  Str::limit($data->judul,30, '...') }}</td>
                            <td>{{  Str::limit($data->desk,50, '...') }}</td>
                            <td>{{$data->prep_time}}</td>
                            <td>{{$data->cook_time}}</td>
                            <td>{{$data->kesulitan}}</td>
                            <td id="action-31">
                                <a href="{{route('recipe.edit', $data->id)}}" class="btn btn-info btn-sm"><i class="fas fa-pencil-alt" aria-hidden="true"></i>&nbsp;&nbsp;Ubah</a>
                                <form id="deleteForm-{{$data->id}}" action="{{route('recipe.destroy', $data->id)}}" style="display:inline-block" method="POST" class="deleteForm-{{$data->id}}">
                                    @method('DELETE')
                                    @csrf
        ...
        {{$recepies->links('pagination::simple-bootstrap-5')}}
        <!-- /.card -->
    </div>
</div>
